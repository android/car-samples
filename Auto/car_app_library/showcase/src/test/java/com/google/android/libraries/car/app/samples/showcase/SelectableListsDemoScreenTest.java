...
/** Tests for the {@link SelectableListsDemoScreen}. */
@RunWith(RobolectricTestRunner.class)
public final class SelectableListsDemoScreenTest {
  private final TestCarContext testCarContext =
      TestCarContext.createCarContext(ApplicationProvider.getApplicationContext());

  @Test
  public void selectRow_callsExpectedCallback() {
    SelectableListsDemoScreen screen = new SelectableListsDemoScreen(testCarContext);

    ListTemplateController controller =
        ListTemplateController.of((ListTemplate) screen.getTemplate());

    controller.getItemListByHeader("Sample selectable list").select(1);

    assertThat(
            testCarContext.getCarService(TestAppManager.class).getToastsShown().get(0).toString())
        .isEqualTo("Changed selection to index: 1");
  }
}

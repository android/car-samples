...
public class PlaceFinder {
  ...
  public List<PlaceInfo> getPlacesByCategory(
      Location location, double radius, int maxCount, String category) {
  ...
  public List<PlaceInfo> getPlacesByName(
      Location location, double radius, int maxCount, String name) {
   ...
   */
  private URL makeSearchURL(Location location, double radius, String searchTerm, boolean isCategory)
  ...
  private String makePhotoURL(String photoReference) throws MalformedURLException {
    return new URL(
            PHOTO_URL + "maxwidth=400" + "&photoreference=" + photoReference + "&key=" + mApiKey)
        .toString();
  }
  ...
  private URL makeDetailsURL(String placeId) throws MalformedURLException {
    return new URL(
        DETAILS_URL
            + "place_id="
            + placeId
            + "&fields=name,rating,formatted_phone_number,geometry,place_id,geometry,photo,icon"
            + "&key="
            + mApiKey);
  }
  ...
  private static String streamToString(InputStream inputStream) throws IOException {
    ...
    if (inputStream != null) {
      BufferedReader reader =
          new BufferedReader(new InputStreamReader(inputStream, Encoding.UTF_8.toString()));

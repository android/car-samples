...
class UiExecutor {
  ...
  /**
   * A single-threaded {@link ExecutorService} that runs jobs on the thread represented by the given
   * {@link Handler}. If we're on the Handler's thread already, it will run the job immediately. It
   * does not support shutdowns, and will throw if they are invoked.
   */
